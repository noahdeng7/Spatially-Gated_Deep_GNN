...
def eval_metrics(label, y_test_log, preds_log):
    """log1p-space + flow-space metrics for a split, tagged with `label`."""
...
n_neg_train = int(len(y_train) * NEG_SAMPLE_RATE)
neg_idx_train = rng.choice(len(X_train), size=n_neg_train, replace=True)
X_train = np.vstack([X_train, X_train[neg_idx_train]])
y_train = np.concatenate([y_train, np.zeros(n_neg_train, dtype=np.float32)])
print(f"neg_sample_rate={NEG_SAMPLE_RATE}  n_neg_train={n_neg_train}  "
